#!/usr/bin/env python 
"""
Modified by Shai Seger, Apr 2020, add support for gcode output organized by layers
Modified by Mark Clausen, Dec 2019, https://www.facebook.com/mark.a.clausen, to write G0 code between paths and in footer (Lines 91, 2414, 2654, 3167)
...
def _print(*arg):
    st = ""
    for s in arg :
        st += str(unicode(s).encode('unicode_escape'))+" "
    inkex.debug(st)
...
class laser_gcode(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--auto-power",                      action="store", type="inkbool",         dest="auto_power",                          default=True,                          help="Set S# by curve color")
        self.OptionParser.add_option("",   "--layer-filter",                    action="store", type="string",          dest="layer_filter",                        default="",                             help="Filter layers processes by gcode")
        ...
        self.OptionParser.add_option("",   "--area-fill-angle",                 action="store", type="float",           dest="area_fill_angle",                     default="0",                            help="Fill lines angle.")                
        self.OptionParser.add_option("",   "--area-fill-shift",                 action="store", type="float",           dest="area_fill_shift",                     default="0",                            help="")                
        self.OptionParser.add_option("",   "--area-fill-thick",                 action="store", type="float",           dest="area_fill_thick",                     default="1",                            help="Pen thickness.")                
        self.OptionParser.add_option("",   "--area-fill-overlap",               action="store", type="float",           dest="area_fill_overlap",                   default="0",                            help="Pen lines overlap.")                
        ...
        self.OptionParser.add_option("",   "--pen-offset-x",                    action="store", type="float",           dest="pen_offset_x",                        default="0",                            help="Pen x offset from head center")                
        self.OptionParser.add_option("",   "--pen-offset-y",                    action="store", type="float",           dest="pen_offset_y",                        default="0",                            help="Pen y offset from head center")                
...
#                    print_((-f(w[k][i-1]),-f(w[k][i]), [i1[5] for i1 in l1]) )
    ...
    def draw_csp(self, csp, layer=None, group=None, fill='none', stroke='#178ade', width=0.354, style=None):
        ...
        if group==None and layer==None:
            ...
            group = layer
        csp = self.apply_transforms(group, csp, reverse=True) #, reverse=True
        if style!=None :
...
################################################################################
    def generate_gcode(self, curve, layer, depth):
        ...
        layername = None
        rematch = None
        use_pen_offsets = False

        for attr in layer.attrib:
            if 'label' in attr:
                layername = layer.attrib[attr]
        if not (layername is None):
            filter = self.options.layer_filter.strip()
            if len(filter) > 0:
                filter = ".*(" + filter + ")"
                if (re.match(filter, layername) is None):
                    return ""
            if re.match('.*AnSkip.*', layername):
                return ""
            rematch = re.match('.*An(\d+).*', layername)
            use_pen_offsets = "Pen" in layername
            #_print(layername)
        if not (rematch is None) and self.options.auto_power:
            tool["gcode before path"] = self.options.laser_command + " S" + str(int(rematch.group(1))) + "\nG4 P" + self.options.power_delay
        #_print("Laser_power=", self.options.laser_power)
        ...
        #    Creating Gcode for curve between s=curve[i-1] and si=curve[i] start at s[0] end at s[4]=si[0]
            ...
            xyloc = si[0]
            if use_pen_offsets:
                xyloc = [xyloc[0] + self.options.pen_offset_x, xyloc[1] + self.options.pen_offset_y]
            ...
            if s[1]    == 'move':
                ...
                if (r[0]**2 + r[1]**2)>.1:
                    ...
                    if abs(r1.mag()-r2.mag()) < 0.001 :
                        g += ("G2" if s[3]<0 else "G3") + c(xyloc+[ None, (s[2][0]-s[0][0]),(s[2][1]-s[0][1])  ]) + "\n"
                    else:
                        r = (r1.mag()+r2.mag())/2
                        g += ("G2" if s[3]<0 else "G3") + c(xyloc) + " R%f" % (r) + "\n"
                    lg = 'G02'
                else:
                    g += "G1 " + c(xyloc) + " " + feed + "\n"
                    lg = 'G01'
...
################################################################################
    ...
    def area_fill(self):
        ...
        if len(self.selected_paths)<=0:
            _print("This function requires at least one selected path.")
            return
        for layer in self.layers :
            if layer in self.selected_paths :
                if self.options.area_fill_thick <= 0 : 
                    self.error(_("Pen diameter must be > 0!") % layer.get(inkex.addNS('label','inkscape')),"area_tools_diameter_error")
                for path in self.selected_paths[layer]:
                    lines = []
                    ...
                    # Zig-zag
                    r = self.options.area_fill_thick*(1-self.options.area_fill_overlap)
                    if r<=0 :
                    ...
                    #_print(self.options.area_fill_method)
...
################################################################################
    def orientation(self, layer=None) :
        ...
        str_height = self.document.getroot().get('height')
        if (str_height == None):
            str_height = "100%"
        ...
        # the unit system is quite a mess, so here is how to handle it.
        # 1. get the height. it should have units on it, if not consider as px
        # 2. convert height to mm.
        # 3. get the view box. 
        # 4. set the scale by dividing the viewbox height by height


        vbox = self.document.getroot().get('viewBox')
        if not (vbox is None):
            doc_height = float(vbox.split(' ')[3])
        re_res = re.match('(([-+]?[0-9]+(\.[0-9]*)?|[-+]?\.[0-9]+)([eE][-+]?[0-9]+)?)(.*)', str_height)
        uu_docheight = float(re_res.group(1))
        uu_units = re_res.group(5)
        if (uu_units == ''):
            uu_units = 'px'
        if uu_units == '%':
            uu_docheight = uu_docheight * doc_height / 100.0
            uu_units = 'px'
        if uu_units == 'in':
            uu_docheight = uu_docheight * 25.4
        elif uu_units == 'mm':
            pass
        else:
            #assume pixels
            uu_docheight = (uu_docheight / self.dpi) * 25.4

        orientation_scale = doc_height / uu_docheight
            
        #_print(str_height, uu_docheight, uu_units, orientation_scale, self.getDocumentUnit())
        ...
        #_print("Document height: ", doc_height, self.getDocumentUnit(), " Scale:", orientation_scale)

        points = points[:2]

        print_(("using orientation scale",orientation_scale,"i=",points))
        #_print("using orientation scale",orientation_scale,"i=",points)
        for i in points :
            # X == Correct!
            ...
            #_print_print(str(-si[1]-10+doc_height))
...
################################################################################
    def effect(self) :
        ...
        if ((inkscape_version is None) or (inkscape_version.find("0.91",0,4) == 0)):
            self.dpi = 90.0
        else:
            self.dpi = 96.0
        #_print(str(self.dpi), inkscape_version)
        ...
        #_print(self.orientation_points)
